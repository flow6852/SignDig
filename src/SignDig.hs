...
customPlotPath scale func legend rat png = plotPathStyle attribute plotstyle z
 where
  x         = linearScale 1000 scale
  y         = fmap func x
  z         = zip x y
  ratio     = if rat == "" then 1 else read rat
  fontType  = "IPAGothic"
  ...
  font      = [tics, xlabel, ylabel, keyFont, titleFont]
  attribute = save ++ key ++ label ++ title ++ size ++ font
  plotstyle = defaultStyle {lineSpec = CustomStyle [LineTitle legend, LineWidth 2.0]}
...
main = do
 ...
 putStr "diff Relative error ?[y/N]:"
 hFlush stdout
 rans <- getLine
 putStr "filename :"
 hFlush stdout
 ...
 decidePdfFile gfn >>= 
  if rans == "y" then do
   customPlotPaths (dba, (read b :: Double)) [dFunc1 dba, dFunc2 dba, dFunc3 dba] 
                                             ["(a+b)/2 - a/2+b/2", "(a+b)/2 - a+(b-a)/2", "a/2+b/2 - a+(b-a)/2"] grt
  else customPlotPaths (dba, (read b :: Double)) [func dba, func1 dba, func2 dba] ["(a+b)/2", "a/2+b/2", "a+(b-a)/2"] grt
  where
   printDetail list = do
    putStrLn "a/2 + b/2"
   ...
   diffFunc funca funcb a b = funca a b - funcb a b
   dFunc1 a b = diffFunc func func1 a b
   dFunc2 a b = diffFunc func func2 a b
   dFunc3 a b = diffFunc func1 func2 a b
